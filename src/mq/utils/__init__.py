...
def format_timestamp_headers(timestamps) -> dict[datetime, str]:
    ...
    if num_unique_dates > 1 and has_multiple_times_in_date:
        fmt_ = fmt_date_time  # case ReportLevel.DIRECTORY: Multiple dates AND multiple times within dates
        log.debug("- case 1")
    elif num_unique_dates > 1:
        fmt_ = fmt_date  # case ReportLevel.FILE: Multiple dates but only 1 timestamp per date
        log.debug("- case 2")
