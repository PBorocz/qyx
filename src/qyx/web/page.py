...
def get_navbar(args: Namespace, active_page: str = None) -> list[dict[str, str]]:
    """Get content for our navbar (basically a list of tools available)."""
    navbars = []
    for tool_name in args.config.get("renderers.web.ui.tool_order"):
        navbars.append({"name": tool_name, "description": tool_name.title()})
    return navbars
...
def render_page(title: str, template: str, **context):
    """Render the specified page template using the context provided."""
    print(f"render_page: {title}")
    print(f"render_page: {request=}")
    # print(f"render_page: {request.app.args=}")
    context["title"] = title
    context["navbar"] = get_navbar(request.app.args)
    template = request.app.args.jinja_env.get_template(template)
    return template.render(**context)


def render_partial(template: str, **context):
    """Render the specified page template using the context provided."""
    template = request.app.args.jinja_env.get_template(template)
    return template.render(**context)


# def render_page_fasthtml(request, title, active_page, *main_page_content):
