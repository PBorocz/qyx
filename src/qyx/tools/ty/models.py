...
#     # We do this as there are valid cases when there are NO Ty table entries for a particular
#     # scan. We still want the timestamp back with a Ty count of *0*.
...
#         return result
#     weights = args.config.get("tools.ty.weighted_violations_per_kloc.weights")
...
#         args,
#         "tools.ty.weighted_violations_per_kloc",
#         metric_value,
#     )
#     return result
...
#     """Return the count of ty results for the scan by rule code prefix."""
#     rows = (
#         Ty.select(
#             fn.SUBSTR(Ty.rule_code, 1, 1).alias("rule_prefix"),
#             fn.COUNT(Ty.id).alias("count"),
#         )
#         .where(
#             Ty.scan == scan,
